...
# 推理函数
# 把输入字符串 prompt 分词成 tensor（张量），并自动填充（padding）   "pt"返回 PyTorch Tensor（最常用）
def generate_response(prompt):
    ...
    with torch.no_grad():   # 临时作用域，用于临时关闭梯度计算（提高速度，节省内存）
        outputs = model.generate(
            ...
            max_length=80,
            do_sample=True,   # 允许随机采样（非贪婪搜索）
            top_k=40,  # 只在概率最高的前 40 个 token 中采样
            top_p=0.9,  # 或选择累计概率 90% 的 token 范围内采样
            temperature=0.7,  # 控制随机程度（越小越保守）
            repetition_penalty=1.2,  # 惩罚重复内容
            pad_token_id=tokenizer.eos_token_id  # 指定填充 token 避免报错
        )
    return tokenizer.decode(outputs[0], skip_special_tokens=True)
    # 把输出的 token 转换为人类可读的字符串（跳过 [EOS] 等特殊符号）
